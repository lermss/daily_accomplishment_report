...
class AdminDashboardController extends Controller
{
    ...
    /**
     * Lightweight JSON endpoint polled by the admin dashboard every 20 s
     * to detect new submissions / status changes without a full page reload.
     */
    public function reportsPoll(Request $request): JsonResponse
    {
        $user = $this->authenticatedUser($request);
        ...
        // Timestamp of the most recent pending report (used by the client
        // to detect when a brand-new submission has arrived).
        $latestPendingAt = \App\Models\Report::query()
            ->where('status', 'pending')
            ->where('is_hidden_from_admin_dashboard', false)
            ->when($user->role === 'ph-admin', function ($q) use ($user) {
                $q->where(function ($sq) use ($user) {
                    $sq->where('assigned_provincial_head_id', $user->id)
                       ->orWhereHas('user', fn ($u) => $u->where('office', $user->office));
                });
            })
            ->max('submitted_at');

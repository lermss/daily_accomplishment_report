...
class AdminPortalService
{
    ...
    public function userFormOptions(): array
    {
        return [
            'hr-super-admin' => [
                ...
                'fields' => ['name', 'email', 'project', 'bureau'],
            ...
            'ph-admin' => [
                ...
                'fields' => ['name', 'email', 'division', 'office'],
            ...
            'staff' => [
                'label' => 'Staff',
                'fields' => ['name', 'email', 'project', 'bureau', 'office'],
    ...
    public function buildDashboardData(Request $request, User $user, string $mode = 'dashboard'): array
    {
        ...
        $users = $this->managedUsersQuery($search, $scope, $roleFilter)
            ->orderBy('name')
            ->paginate(max(5, min(100, (int) $request->query('per_page', 15))), [
        ...
        // PH Admins by Office — for the new horizontal bar chart
        $phAdminByOffice = DB::table('users')
            ->where('role', 'ph-admin')
            ->where('status', 'active')
            ->whereNotNull('office')
            ->groupBy('office')
            ->orderByDesc(DB::raw('COUNT(*)'))
            ->get(['office', DB::raw('COUNT(*) as count')])
            ->pluck('count', 'office')
            ->toArray();
        ...
        $pendingReports = (int) DB::table('reports')
            ...
            ->where('is_hidden_from_admin_dashboard', false)
            ->count();
        $approvedReports = (int) DB::table('reports')
            ->where('status', 'approved')
            ->where('is_hidden_from_admin_dashboard', false)
            ->count();
        $forRevisionReports = (int) DB::table('reports')
        ...
        return [
            ...
            'kpiCards' => [
                ...
                [
                    'key'   => 'for_revision',
                    'label' => 'For Revision',
                    'count' => $forRevisionReports,
                    'meta'  => 'Returned for updates',
                    'tone'  => 'rose',
                    'icon'  => 'revision',
                ],
            ],
    ...
    private function adminReportsQuery(?User $user = null, string $search = '', ?string $status = null, string $dateFilter = '')
    {
        return DB::table('reports')
            ...
            ->when($user?->role === 'ph-admin', function ($query) use ($user) {
                // Show reports assigned to this ph-admin OR from their office (consistent with scopeReportsForReviewer)
                $query->where(function ($scopedQuery) use ($user) {
                    $scopedQuery
                        ->where('reports.assigned_provincial_head_id', $user->id)
                        ->orWhere('users.office', $user->office);
                });
            })

...
class Report extends Model
{
    ...
    public function markAsReviewed(string $status, int $reviewerId, ?string $comment = null): void
    {
        ...
        // When approved: ensure the report is visible on the staff's Dashboard AND Reports page
        // so they see the updated "Approved" status instead of the old "For Revision".
        if ($status === self::STATUS_APPROVED) {
            $updates['is_hidden_from_staff_dashboard'] = false;
            $updates['is_hidden_from_staff_index']     = false;
        }

        // When returned for revision: unhide from the reports index so staff can find and edit it,
        // but leave the dashboard flag alone (staff may have already dismissed it there).
        if ($status === self::STATUS_FOR_REVISION) {
            $updates['is_hidden_from_staff_index'] = false;
        }

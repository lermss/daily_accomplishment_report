(() => {
    ...
    // ── Centered review-result alert modal ──────────────────────────────────
    function showReviewAlert(message, type) {
        // Remove any existing alert
        const existing = document.getElementById('review-alert-modal');
        if (existing) existing.remove();

        const isSuccess = type === 'success';
        const overlay = document.createElement('div');
        overlay.id = 'review-alert-modal';
        ...
        box.style.cssText = [
            ...
            'border:1.5px solid ' + (isSuccess ? '#22c55e' : '#f59e0b'),
            ...
            'max-width:380px',
            'text-align:center',
            'animation:reviewAlertIn 0.25s ease'
        ].join(';');

        const icon = document.createElement('span');
        icon.style.cssText = 'font-size:1.4rem;line-height:1;flex-shrink:0';
        ...
        closeBtn.style.cssText = [
            ...
            'cursor:pointer', 'font-size:1rem', 'color:inherit', 'opacity:0.6',
            'padding:0 0 0 8px', 'flex-shrink:0'
        ].join(';');
        closeBtn.addEventListener('click', () => overlay.remove());

        // Add keyframe once
        if (!document.getElementById('review-alert-keyframes')) {
            const style = document.createElement('style');
            style.id = 'review-alert-keyframes';
            style.textContent = '@keyframes reviewAlertIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:none}}';
    ...
    async function submitReview(status) {
        ...
        // Disable the active button and show a spinner — no inline text
        const activeBtn = status === 'approved' ? approveButton : returnButton;
        ...
        try {
            ...
            // ── Restore button, then show centered alert modal ────────────
            if (activeBtn) {
                activeBtn.disabled = false;
                activeBtn.textContent = originalLabel;
            }
            const alertMsg = updatedStatus === 'approved'
                ? 'Report approved successfully!'
                : 'Report returned for revision.';
            showReviewAlert(alertMsg, updatedStatus === 'approved' ? 'success' : 'warning');
            if (reviewFeedback) reviewFeedback.textContent = '';
            ...
            showReviewAlert('Unable to save the review right now. Please try again.', 'warning');
            if (reviewFeedback) reviewFeedback.textContent = '';
        }
    }

        ...
        <section class="table-panel">
            <div class="table-toolbar">
                <div class="report-filters-row">
                    <div class="filters">
                        <form method="GET" action="{{ route($staffRouteBase . '.reports') }}" class="search-form report-search-form" id="reportSearchForm" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                            ...
                            <select name="status_filter" aria-label="Filter by status"
                                onchange="document.getElementById('reportSearchForm').submit()"
                                style="height:42px;padding:0 14px;border:1.5px solid #e5e7eb;border-radius:50px;
                                       font-family:inherit;font-size:.85rem;background:#fff;color:#374151;
                            ...
                            >
                                ...
                                <option value="for_revision"{{ request('status_filter') === 'for_revision' ? 'selected' : '' }}>For Revision</option>
                            </select>
                            ...
                            >
                                @foreach ([10, 20, 50] as $opt)
                                    <option value="{{ $opt }}" {{ ($perPage ?? 10) == $opt ? 'selected' : '' }}>{{ $opt }} per page</option>
                                @endforeach
                            </select>

                            @if(request()->filled('search') || request()->filled('status_filter'))
                                <a href="{{ route($staffRouteBase . '.reports') }}"
                                   style="padding:0 18px;height:42px;display:inline-flex;align-items:center;
                                          border:1.5px solid #e5e7eb;border-radius:50px;font-family:inherit;
                                          font-size:.85rem;color:#374151;text-decoration:none;background:#fff;">
                                    Clear
                                </a>
                            @endif
                        </form>
...
{{-- Bottom-right success toast for delete/edit --}}
@if(session('success'))
<div id="reportsToast" style="
...
">
    ...
    <button onclick="document.getElementById('reportsToast').remove()" style="background:none;border:none;color:rgba(255,255,255,.8);font-size:1.1rem;cursor:pointer;margin-left:8px;">×</button>
</div>
<style>
@keyframes slideInRight { from { opacity:0; transform:translateX(40px); } to { opacity:1; transform:translateX(0); } }
</style>
<script>setTimeout(() => { const t = document.getElementById('reportsToast'); if(t) t.remove(); }, 5000);</script>
@endif

@php
    $adminNavbarActive = $active ?? 'home';
    $adminNavbarUser = $user ?? null;
    $adminNavbarCanAccessAudit = $canAccessAudit ?? false;
    $authFlowService = app(\App\Services\AuthFlowService::class);
    $isAdminNavigation = $adminNavbarUser !== null && $authFlowService->isAdminRole($adminNavbarUser->role);
    $isSuperAdminNavigation = $adminNavbarUser !== null && $authFlowService->isSuperAdminRole($adminNavbarUser->role);
    $canViewNotifications = $isAdminNavigation || $isSuperAdminNavigation;
    $reportsRoute = $isSuperAdminNavigation ? route('reports.index') : route('admin.dashboard.employees');
    ...
    if ($canViewNotifications) {
        $submissionNotifications = \Illuminate\Support\Facades\DB::table('reports')
            ...
            ->when($adminNavbarUser?->role === 'ph-admin', function ($query) use ($adminNavbarUser) {
                $query->where(function ($scopedQuery) use ($adminNavbarUser) {
                    $scopedQuery
                        ->where('reports.assigned_provincial_head_id', $adminNavbarUser->id)
                        ->orWhere(function ($fallbackQuery) use ($adminNavbarUser) {
                            $fallbackQuery
                                ->whereNull('reports.assigned_provincial_head_id')
                                ->where('users.office', $adminNavbarUser->office);
                        });
                });
            })
...
<header class="navbar">
    ...
    <nav class="nav-right" aria-label="Primary">
        <a href="{{ route('dashboard.home') }}" class="{{ $adminNavbarActive === 'home' ? 'active' : '' }}">Home</a>
        <a href="{{ route('dashboard') }}" class="{{ $adminNavbarActive === 'dashboard' ? 'active' : '' }}">Dashboard</a>
        @if ($isSuperAdminNavigation)
            <a href="{{ $reportsRoute }}" class="{{ $adminNavbarActive === 'reports' ? 'active' : '' }}">Reports</a>
        @endif
        @if ($adminNavbarCanAccessAudit || $isAdminNavigation)
            <a href="{{ route('audit.index') }}" class="{{ $adminNavbarActive === 'audit' ? 'active' : '' }}">Audit Log</a>
        @endif

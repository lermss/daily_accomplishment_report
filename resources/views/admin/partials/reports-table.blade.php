<section class="table-panel">
    ...
    <form method="GET" action="{{ url()->current() }}" id="reportFilterForm">
        <div class="report-filters-row">
            <div class="filters-left">
                ...
                <select name="per_page" class="status-select" aria-label="Rows per page" onchange="this.form.submit()" style="min-width:130px;">
                    @foreach ([10, 20, 50, 100] as $opt)
                        <option value="{{ $opt }}" {{ (int) request('per_page', 15) === $opt ? 'selected' : '' }}>{{ $opt }} per page</option>
                    @endforeach
                </select>
            </div>
            <div class="filters-right">
                <button type="button" id="deleteSelectedBtn" class="btn btn-danger"
                        style="padding: 8px 16px; border-radius: 6px;" disabled>Delete Selected</button>
            </div>
    ...
    {{-- ── Bulk delete form wrapping scrollable table ──────── --}}
    <form id="bulkDeleteForm" method="POST"
          action="{{ $isSuperAdminView ? route('reports.bulk-delete') : route('admin.dashboard.bulk-delete') }}">
        @csrf
            ...
            <table>
                ...
                <tbody data-reports-body>
                    @forelse ($reports as $report)
                        ...
                        @endphp
                        <tr data-report-row data-report-id="{{ $report->id }}"
                            data-status="{{ $report->status }}"
                            data-search="{{ strtolower(trim(($report->user_name ?: 'unassigned user') . ' ' . ($report->file_name ?: '') . ' ' . ($report->status ?: ''))) }}">
                            <td>
                                <input type="checkbox" name="report_ids[]" value="{{ $report->id }}"
                                       class="report-checkbox" {{ $report->status !== 'approved' ? 'disabled' : '' }}>
                            </td>
                            <td>
                                <div class="name-cell">
                                    @if ($avatarUrl)
                                    ...
                                    @endif
                                    <div class="name-copy">
                                        <strong>{{ $report->user_name ?: 'Unassigned User' }}</strong>
                            ...
                            <td>
                                <span class="status-pill {{ $statusClass }}" data-status-pill>
                                    {{ ucfirst(str_replace('_', ' ', $report->status ?? 'unknown')) }}
                                </span>
                            </td>
                            <td>
                                <div class="action-stack">
                                    <button type="button" class="action-button" data-open-report-modal
                                    ...
                                    @if ($report->file_path && ! $isSuperAdminView)
                                        <a href="{{ asset($report->file_path) }}" class="icon-action" download title="Download report">
                                            <svg viewBox="0 0 24 24"><path d="M12 3a1 1 0 0 1 1 1v8.59l2.3-2.29 1.4 1.41L12 16.41l-4.7-4.7 1.4-1.41L11 12.59V4a1 1 0 0 1 1-1Zm-7 14h14v3H5Z"/></svg>
                                        </a>
                                    @endif
                                </div>
                            </td>

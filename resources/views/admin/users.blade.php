...
@push('styles')
    ...
    <style>
        ...
        .role-intern { background: #fef3c7; color: #b45309; }
            ...
            <section class="dashboard-content">
                ...
                <section class="table-panel" style="margin-top: 15px">
                        ...
                        <div class="toolbar-main-actions">
                            <form method="GET" action="{{ url()->current() }}" class="search-filter-bar" data-search-filter-form>
                                ...
                                <div class="toolbar-filter-actions">
                                    ...
                                    <label class="filter-select">
                                        <span>Per page</span>
                                        <select name="per_page" aria-label="Rows per page" onchange="this.closest('form').submit()">
                            ...
                            <tbody>
                                @forelse ($users as $listedUser)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $roleLabel = match($listedUser->role) {
                                                    'staff'          => 'Staff',
                                                    'interns'        => 'Intern',
                                                    'ph-admin'       => 'PH Admin',
                                                    'hr-super-admin' => 'HR Super Admin',
                                                    default          => ucwords(str_replace(['-','_'], ' ', $listedUser->role ?? 'N/A')),
                                                };
                                                $roleTone = match($listedUser->role) {
                                                    'hr-super-admin' => 'role-super',
                                                    'ph-admin'       => 'role-admin',
                                                    'staff'          => 'role-staff',
                                                    'interns'        => 'role-intern',
                    ...
                    @if ($users instanceof \Illuminate\Pagination\LengthAwarePaginator && $users->hasPages())
                        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;padding:14px 4px;border-top:1px solid #f1f5f9;margin-top:4px;">
                            <div style="font-size:13px;color:#6b7280;font-weight:500;">
                                Showing {{ $users->firstItem() }}–{{ $users->lastItem() }} of {{ $users->total() }} users
                            </div>
                            <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                                @if ($users->onFirstPage())
                                    <span class="up-btn up-btn--disabled">&laquo;</span>
                                @else
                                    <a href="{{ $users->previousPageUrl() }}" class="up-btn">&laquo;</a>
                                @endif
                                @foreach ($users->getUrlRange(max(1,$users->currentPage()-2), min($users->lastPage(),$users->currentPage()+2)) as $page => $url)
                                    @if ($page == $users->currentPage())
                                        <span class="up-btn up-btn--active">{{ $page }}</span>
                                    @else
                                        <a href="{{ $url }}" class="up-btn">{{ $page }}</a>
                                    @endif
                                @endforeach
                                @if ($users->hasMorePages())
                                    <a href="{{ $users->nextPageUrl() }}" class="up-btn">&raquo;</a>
                                @else
                                    <span class="up-btn up-btn--disabled">&raquo;</span>
                                @endif
                            </div>
                        </div>
                        <style>
                        ...
                        .up-btn:hover { background:#f0f5ff;border-color:#a5b4fc;color:#4338ca; }
                        .up-btn--active { background:linear-gradient(135deg,#4f46e5,#6366f1);border-color:transparent;color:#fff;cursor:default; }
                        .up-btn--active:hover { background:linear-gradient(135deg,#4f46e5,#6366f1);color:#fff; }
                        .up-btn--disabled { opacity:.35;cursor:not-allowed;pointer-events:none; }
                        </style>
                    @endif
                </section>
            </section>
